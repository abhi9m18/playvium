...
export default function SidehustlesTable() {
    ...
    <div className="w-full mt-6 px-0 mb-5">
      ...
      <div className="w-full mb-4">
        ...
        <div className="md:hidden flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-2 bg-[#14171D] p-1 border border-[#242B35] rounded-2xl">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`relative px-4 py-2 rounded-xl text-xs font-medium transition-all w-full text-center ${
                  activeTab === tab.id
                    ? "bg-[#B93DEB] text-white shadow-[0_0_10px_rgba(185,61,235,0.6)]"
                    : "text-white/60 hover:text-white"
                }`}
              >
                ...
                {tab.dot && (
                  <>
                    <span className="absolute top-1/2 -translate-y-1/2 right-2 w-2 h-2 rounded-full bg-pink-400 animate-ping"></span>
                    <span className="absolute top-1/2 -translate-y-1/2 right-2 w-2 h-2 rounded-full bg-pink-500"></span>
                  </>
                )}
              </button>
            ))}

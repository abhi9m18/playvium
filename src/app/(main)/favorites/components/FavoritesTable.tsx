...
import {  fortuneIcon } from "@/assets/images/favorites";
...
export default function FavoritesTable() {
  ...
  useEffect(() => {
    ...
    const offsets = tabRefs.current.map((el) => el?.offsetLeft || 0);
    ...
    <div className="w-full mt-8 mb-5">
      ...
      <div className="w-full mb-4">
        ...
        <div className="hidden md:flex justify-between items-center w-full">
          <div className="relative flex items-center bg-[#14171D] p-1 border border-[#242B35] rounded-2xl">
            {/* Sliding Background */}
            <div
              className="absolute top-1 h-[calc(100%-8px)] bg-[#B93DEB] rounded-xl transition-all duration-300 shadow-[0_0_10px_rgba(185,61,235,0.6)]"
              style={{
                width: tabWidths[activeIndex] || 0,
                transform: `translateX(${tabOffsets[activeIndex] || 0}px)`,
              }}
            ></div>
            ...
            {tabs.map((tab, i) => (
              <button
                ...
                className={`relative z-10 px-6 py-2.5 rounded-xl text-sm font-medium transition-all 
                  ${
                    activeTab === tab.id
                      ? "text-white"
              ...
              >
                ...
                {tab.dot && (
                  <>
                    <span className="absolute right-2 ml-2 top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full bg-pink-400 animate-ping"></span>
                    <span className="absolute right-2 ml-2 top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full bg-pink-500"></span>
                  </>
                )}
              </button>
            ))}
        ...
        <div className="md:hidden flex flex-col gap-3">
          <div className="flex overflow-x-auto whitespace-nowrap  scrollbar-hide bg-[#14171D] p-1 border border-[#242B35] rounded-2xl">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`relative px-4 py-2 rounded-xl text-xs font-medium  transition-all 
                  ${
                    activeTab === tab.id
                      ? "bg-[#B93DEB] text-white shadow-[0_0_6px_rgba(185,61,235,0.6)]"
                      : "text-white/60"
                  }`}
              >
                {tab.label}
                ...
                {tab.dot && (
                  <>
                    <span className="absolute right-1 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-pink-400 animate-ping"></span>
                    <span className="absolute right-1 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-pink-500"></span>
                  </>
                )}
              </button>
            ))}
            ...
            filteredData.map((row) => (
              ...
              >
                <div className="flex items-center gap-1.5 pr-2">
                  <Image src={fortuneIcon} alt="icon" className="w-4 h-5" />
                  {row.game}
                </div>

                <span className="pl-4">{row.user}</span>
                <span>{row.time}</span>
          ...
          filteredData.map((row) => (
            ...
            >
              <div className="w-[8%] flex items-center justify-center">
                <Image src={fortuneIcon} alt={row.game} className="w-10 h-10 object-contain" />
              </div>
              ...
              <div className="flex flex-col w-[35%]">
                ...
                <div className="flex items-center gap-2 text-white/60 text-[9px] mt-0.5">
              ...
              <div className="flex items-center justify-between w-[46%] text-[10px]">
                <div className="flex flex-col w-[33%]">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-yellow-400">{row.buyIn}</span>
                  </div>
                </div>

                <div className="flex flex-col text-center w-[40%] ">
                  <span className="text-white">Multiplier</span>
                  <span className="text-white">{row.multiplier}</span>
                </div>

                <div className="flex flex-col text-center  w-[37%]">
                  <span className="text-white">Win/Loss</span>
                  <div className="flex items-center gap-1">
                    <Image src={coingreen} alt="profit" className="w-3 h-3" />
                    <span className="text-[#03B739] font-medium truncate">
                      {row.winLoss}
                    </span>
                  </div>
                </div>
